...
public class JwtProvider {
	...
	public String resolveRefreshToken(@Nullable HttpServletRequest request) {
		...
		Cookie refreshTokenCookie = Arrays.stream(request.getCookies())
			.filter(cookie -> cookie.getName().equals(HeaderConstants.REFRESH_TOKEN_HEADER))
			.findAny()
			.orElse(null);
	...
	public <T> T extractClaim(String token, boolean isAccessToken, Function<Claims, T> claimResolver) {
	...
	public boolean isTokenValid(String token, boolean isAccessToken, MemberDetails memberDetails) {
		try {
			...
			return (!isTokenExpired(token, isAccessToken) && memberId.equals(memberDetails.getUsername()));
